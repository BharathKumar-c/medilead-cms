  ...
  calls: [
    {
      ...
      auth: 'HMAC / JWT',
      description: 'Telephony vendor webhook — record inbound call with HMAC signature verification',
      body: {
        caller_phone_number: 'string (E.164, required)',
        call_status: 'string (required: initiated|ringing|in-progress|completed|failed|missed)',
        timestamp: 'ISO8601 (required)',
        vendor_call_id: 'string (optional, max 100 chars — deduplication key)',
        duration_seconds: 'integer (optional, min 0)',
        direction: 'string (optional, inbound|outbound, default inbound)',
        recording_url: 'string (optional, max 500 chars)',
        intercom_number: 'string (optional, max 50 chars — agent routing)',
      },
    },
    {
      method: 'POST',
      ...
      auth: 'Required',
      description: 'SIP event webhook — process SIP server events with auto lead lookup',
      body: {
        event: 'string (required: incoming|outgoing|answered|ended|missed|hold|unhold|failed)',
        call_id: 'string (optional, max 100 chars — for updating existing calls)',
        caller: 'string (optional, 10-15 digits)',
        callee: 'string (optional, 10-15 digits)',
        duration: 'integer (optional, min 0)',
      },
    },
    {
      method: 'GET',
      path: '/api/calls',
      auth: 'Required',
      description: 'List call logs with pagination, filtering, and intercom scoping',
    ...
    },
    {
      ...
      auth: 'Required',
      description: 'Get today call statistics (total, inbound, outbound, missed, avg duration) — scoped by intercom for non-admins',
    },
    {
      method: 'GET',
      path: '/api/calls/phone/:phone',
      auth: 'Required',
      description: 'Get call history for a specific phone number (last 10 calls)',
    },
    {
      method: 'POST',
      path: '/api/calls',
      auth: 'Required',
      description: 'Log a call manually with auto lead linking',
      body: {
        caller_number: 'string (required, 10-15 digits)',
        callee_number: 'string (optional, 10-15 digits)',
        direction: 'string (optional, inbound|outbound, default inbound)',
        status: 'string (optional, default initiated)',
        duration: 'integer (optional, default 0)',
        lead_id: 'integer (optional — skips auto-lookup)',
    ...
    },
    {
      ...
      path: '/api/calls/:id',
      auth: 'Required',
      description: 'Update call status, duration, or notes',
      body: {
        call_status: 'string (optional: initiated|ringing|in-progress|completed|failed|missed)',
        duration_seconds: 'integer (optional, min 0)',
        notes: 'string (optional, max 2000 chars)',
      },
    },
    {
      ...
      auth: 'Required',
      description: 'Telephony-specific backward-compatible list with phone masking',
    ...
    },
    {
      ...
      auth: 'Required',
      description: 'Telephony-specific backward-compatible today stats (snake_case response)',
    },
  ],
  dashboard: [
    {
...
export const databaseTables = [
  ...
  {
    name: 'telephony_call_logs',
    description: 'Unified call records from webhook, SIP events, and manual entry',
    fields: [
      'id',
      'code',
      'caller_phone_number',
